...
The interactive debugging tool made using ipywidget can visualize the game data downloaded in the previous step from JSON files. We can use this tool to browse information for any game in a particular season. We can select the game type, and the particular game, and browse through the data, in a play-by-play fashion.
The tool summarizes the match statistics for each game, such as the goals/shots and shoot-out information, if available. Also, for each event, we plot the event coordinates, period, team side, and other information such as players and teams, related to the event. Here is an example snippet of the tool. 
